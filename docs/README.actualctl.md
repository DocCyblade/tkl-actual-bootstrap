...
## Commands
- `help` — full help
- `list` — instances and symlink targets (and versions under `/srv/app`)
- `check` — basic health checks (tools, services exist)
- `env [INSTANCE]` — show ACTUAL_DATA_DIR, config, link target
- `health [INSTANCE]` — check Nginx upstream (`/health/upstream`) and backend for one or all instances
- `doctor` — full system check (required tools, `nginx -t`, service state, port listening, upstream health)

### Version & app
- `fetch <vX.Y.Z>` : install version to `/srv/app/<ver>` (e.g., `v25.7.1`)
- `switch <INSTANCE> <vX.Y.Z>` : relink and restart (e.g., `switch production v25.7.1`)
- `verify <vX.Y.Z>` : temp install + presence check of module, then remove
